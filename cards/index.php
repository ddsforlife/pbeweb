...
<head>
    ...
    <title>Flashcards - PBE Team Bold</title>
    
    <!-- Favicons -->
    <?php 
    $version = '3.7.3'; // Cache busting + progress removal on recent item dismiss
    ?>
    <link rel="icon" type="image/x-icon" href="favicon.ico?v=<?php echo $version; ?>">
    <link rel="icon" type="image/png" sizes="96x96" href="favicon-96x96.png?v=<?php echo $version; ?>">
    <link rel="apple-touch-icon" sizes="180x180" href="apple-touch-icon.png?v=<?php echo $version; ?>">
    <?php
...
<body data-theme="light">
        ...
        <div class="deck-selection" id="deckSelection">
            <header>
                <div class="logo-section">
                    <a href="/"><img src="pbe_team_bold_logo.png?v=<?php echo $version; ?>" alt="PBE Team Bold Logo" class="logo"></a>
    ...
    <script>
        ...
        // Purge all stored progress for a set of decks (combo key + per-file keys + recentlyUsed)
        function clearDeckProgress(decks) {
            const deckArray = Array.isArray(decks) ? decks : [decks];
            // Remove combined pile data key (handles multi-deck sessions)
            const comboKey = deckArray.slice().sort().join(',');
            localStorage.removeItem(`pileData_${comboKey}`);
            localStorage.removeItem(`progress_${comboKey}`);
            // Remove per-file keys
            deckArray.forEach(file => {
                localStorage.removeItem(`pileData_${file}`);
                localStorage.removeItem(`progress_${file}`);
                localStorage.removeItem(`pileMode_${file}`);
            });
            // Remove from recentlyUsed tracking
            const recentlyUsed = JSON.parse(localStorage.getItem('recentlyUsed') || '{}');
            deckArray.forEach(file => delete recentlyUsed[file]);
            localStorage.setItem('recentlyUsed', JSON.stringify(recentlyUsed));
        }

        function clearRecentStudied() {
            if (confirm('Clear all recently studied history and their progress?')) {
                // Purge progress for every deck in recent history before removing the list
                const recent = getRecentStudied();
                recent.forEach(item => clearDeckProgress(item.decks));
                localStorage.removeItem('recentStudied');
                loadRecentStudied();
                if (DOM.deckSelection.style.display === 'block') loadDecks();
            }
        }

        function removeRecentItem(decks) {
            const recent = getRecentStudied();
            const deckKey = Array.isArray(decks) ? decks.slice().sort().join(',') : decks;
            
            const filtered = recent.filter(item => {
                const itemKey = item.decks.slice().sort().join(',');
                return itemKey !== deckKey;
            });
            
            // Clear stored progress for the removed item's decks
            clearDeckProgress(decks);
        ...
        // Fisher-Yates with crypto random — 3 independent passes to break residual structure
        ...
        function smartShuffle(cards) {
            ...
            cards.forEach(card => {
                const key = card.deckName || 'unknown';
                if (!buckets.has(key)) buckets.set(key, []);
                buckets.get(key).push(card);
            });

            // Shuffle items within each bucket (3-pass crypto)
            for (const [key, arr] of buckets) {
                shuffleArray(arr);
            }

            // Phase 1: Build valid ordering via randomized bucket interleaving
            // Try multiple attempts, collect valid orderings, pick one at random
            const bestOf = [];
            for (let attempt = 0; attempt < 50; attempt++) {
                const result = buildInterleavedOrder(buckets, cards.length);
                if (result) {
                    bestOf.push(result);
                    if (bestOf.length >= 5) break;
                }
            }
            ...
            if (bestOf.length > 0) {
                ...
                // Fallback: flat shuffle
                ordered = [...cards];
                shuffleArray(ordered);
            }

            // Phase 2: Disruption pass — random constraint-safe swaps to eliminate
            // residual patterns from the interleaving algorithm
            return disruptDeckOrder(ordered);
        }

        // Build an interleaved ordering where no deck appears more than 2x in a row
        function buildInterleavedOrder(buckets, totalCards) {
            let bucketKeys = [...buckets.keys()];
            shuffleArray(bucketKeys);
            ...
            const out = [];
            let lastDeck = null;
            let lastLastDeck = null;
            let stuckCount = 0;

            while (out.length < totalCards && stuckCount < 1000) {
                // Find buckets that have cards left and won't create 3-in-a-row
                const available = bucketKeys.filter(k => {
                    if (cursors.get(k) >= buckets.get(k).length) return false;
                    // Block 3-in-a-row from same deck
                    if (lastDeck && lastLastDeck && k === lastDeck && k === lastLastDeck) return false;
                    return true;
                });

                if (available.length === 0) {
                    // Forced pick from whatever's left (constraint-breaking fallback)
                ...
                // Re-shuffle bucket order periodically to avoid patterns
                if (out.length % 7 === 0) shuffleArray(bucketKeys);
            }

            if (out.length < totalCards) return null;
            return out;
        }

        // Disruption pass: random swaps that maintain the no-3-in-a-row constraint
        function disruptDeckOrder(arr) {
            const a = [...arr];
            const len = a.length;
            if (len < 4) return a;
            ...
            for (let t = 0; t < attempts; t++) {
                ...
                if (meetsDeckConstraint(testArr, i) && meetsDeckConstraint(testArr, j)) {
                    [a[i], a[j]] = [a[j], a[i]];
                }
            }
            return a;
        ...
        // Check that position idx doesn't create 3-in-a-row from the same deck
        function meetsDeckConstraint(arr, idx) {
            const deck = arr[idx].deckName || 'unknown';
            // Check triplet: idx-2, idx-1, idx
            if (idx >= 2 && (arr[idx-1].deckName || 'unknown') === deck && (arr[idx-2].deckName || 'unknown') === deck) return false;
            // Check triplet: idx-1, idx, idx+1
            if (idx >= 1 && idx < arr.length - 1 && (arr[idx-1].deckName || 'unknown') === deck && (arr[idx+1].deckName || 'unknown') === deck) return false;
            // Check triplet: idx, idx+1, idx+2
            if (idx < arr.length - 2 && (arr[idx+1].deckName || 'unknown') === deck && (arr[idx+2].deckName || 'unknown') === deck) return false;
            return true;
        }
