...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Print Styles */
        @media print {
            /* Hide EVERYTHING except passage */
            * {
                visibility: hidden !important;
            }
            
            /* Show only passage elements */
            .study-container,
            .study-container *,
            .passage-container,
            .passage-container *,
...
<body>
    ...
    <script>
        ...
        document.getElementById('printFilledBtn').addEventListener('click', () => {
            ...
            // Temporarily render passage with ALL blanks showing correct answers
            const verseTexts = state.currentVerses.map(v => v.text.trim());
            ...
            words.forEach((word, index) => {
                if (blankIndices.has(index)) {
                    const blank = state.blanks.find(b => b.index === index);
                    // Calculate width based on word length
                    const wordLength = blank.word.length || 3;
                    const blankWidth = Math.max(wordLength * 20, 50);
                    // Show the correct answer for ALL blanks with punctuation
