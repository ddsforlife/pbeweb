    ...
    <script>
        ...
        const state = {
            ...
            settings: {
                ...
                randomBlanks: false, // Randomize which words are blanked each session
        ...
        // Text size → per-character blank width (px) and minimum blank width
        const TEXT_SIZE_CONFIG = {
            xs:  { charWidth: 13, minWidth: 36 },
            sm:  { charWidth: 16, minWidth: 42 },
            md:  { charWidth: 20, minWidth: 50 },
            lg:  { charWidth: 24, minWidth: 58 },
            xl:  { charWidth: 28, minWidth: 66 }
        };
            ...
            state.currentVerses.forEach((verse, verseIdx) => {
                ...
                words.forEach((word, wordIdx) => {
                    if (blankIndices.has(globalWordIndex)) {
                        ...
                        // Calculate width based on word length, scaled to current text size
                        const wordLength = blank.word.length || 3;
                        const sizeConf = TEXT_SIZE_CONFIG[state.settings.textSize] || TEXT_SIZE_CONFIG.md;
                        const blankWidth = Math.max(wordLength * sizeConf.charWidth, sizeConf.minWidth);
            ...
            state.currentVerses.forEach((verse, verseIdx) => {
                ...
                words.forEach((word, wordIdx) => {
                    if (blankIndices.has(globalWordIndex)) {
                        const blank = state.blanks.find(b => b.index === globalWordIndex);
                        const wordLength = blank.word.length || 3;
                        const blankWidth = Math.max(wordLength * 14, 40); // Print uses fixed 12pt
