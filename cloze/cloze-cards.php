    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .verse-card {
            ...
            max-width: 800px; width: 100%; min-height: 280px;
        ...
        .fill-btn { font-size: 0.7rem !important; font-weight: 700 !important; letter-spacing: 0.02em; }
        ...
        .shortcuts-grid { display: flex; flex-direction: column; gap: 0.35rem; }
        ...
        .shortcuts-grid kbd {
            ...
            border-radius: 5px; font-family: 'IBM Plex Sans', monospace;
        ...
        .fill-progress-bar {
            height: 6px; background: var(--bg-secondary); flex-shrink: 0;
        }
        .fill-progress-bar .progress-bar {
            height: 100%; background: linear-gradient(90deg, var(--accent), var(--accent-hover));
        ...
        .fill-counter {
            ...
            z-index: 10; background: var(--bg-primary); padding: 0.15rem 0.75rem;
        ...
        .fill-exit-btn:hover { border-color: var(--accent); }
        ...
        /* Desktop/tablet: cap card width for readability */
        @media (min-width: 769px) {
            .fill-card { max-width: 800px; }
        }
        /* Mobile: edge-to-edge */
        @media (max-width: 768px) {
            .fill-card-area { padding: 2.5rem 1rem; }
            .fill-card { border-radius: 12px; padding: 1.25rem; }
        }

        .fill-tap-zone {
            position: absolute; bottom: 80px; height: 30%; width: 12%;
            z-index: 5; cursor: pointer;
            -webkit-tap-highlight-color: transparent;
        }
        .fill-tap-left { left: 0; }
        .fill-tap-right { right: 0; }

        .fill-reveal-bar {
            flex-shrink: 0; display: flex; justify-content: center;
            padding: 0.5rem 1.5rem 1rem; background: var(--bg-primary);
        }
        @media (max-width: 768px) {
            .fill-reveal-bar { padding: 0.5rem 0.75rem 0.75rem; }
        }
        ...
        @media (max-width: 768px) {
            ...
            .reveal-action-btn { font-size: 1rem; padding: 0.75rem; min-height: 50px; }
            ...
            .font-btn[data-size="lg"] { font-size: 0.9rem !important; }
            .fill-btn { font-size: 0.55rem !important; }
            ...
            .card-body { padding-top: 62px; padding-bottom: 75px; min-height: 100vh; }
...
<body>
    <div class="container">
        ...
        <div id="cardStudyScreen">
            ...
            <div class="card-toolbar" id="cardToolbar">
                ...
                <span class="font-size-group" id="fontSizeGroup">
                    ...
                    <button class="toolbar-btn font-btn fill-btn" id="fillModeBtn" title="Fill Screen">Fill</button>
                </span>
        ...
        <!-- Fill Mode Overlay (sibling of cardStudyScreen, not nested inside it) -->
        <div class="fill-overlay" id="fillOverlay" style="display:none;">
            <div class="fill-progress-bar"><div class="progress-bar" id="fillProgressBar"></div></div>
            <div class="fill-counter" id="fillCounter">1 / 1</div>
            <button class="fill-exit-btn" id="fillExitBtn">✕</button>

            <div class="fill-tap-zone fill-tap-left" id="fillTapLeft"></div>
            <div class="fill-tap-zone fill-tap-right" id="fillTapRight"></div>

            <div class="fill-card-area" id="fillCardArea">
                <div class="verse-card fill-card" id="fillCard">
                    <div class="card-reference" id="fillReference"></div>
                    <div class="card-verse-text" id="fillVerseText"></div>
                </div>
            </div>

            <div class="fill-reveal-bar" id="fillRevealBar">
                <button class="reveal-action-btn" id="fillRevealBtn">👁️ Reveal Word</button>
            </div>
        </div>
        ...
        <div class="modal" id="shortcutsModal">
            <div class="modal-content" style="max-width:480px;">
                <h2 class="modal-header">⌨️ Keyboard Shortcuts</h2>
                <div class="shortcuts-grid">
                    <div class="shortcut-section-title">Typing</div>
                    <div class="shortcut-row"><span class="shortcut-desc">Type letter</span><kbd>A–Z</kbd></div>
                    <div class="shortcut-row"><span class="shortcut-desc">Delete letter</span><kbd>Backspace</kbd></div>

                    <div class="shortcut-section-title">Current Blank</div>
                    <div class="shortcut-row"><span class="shortcut-desc">Reveal / skip word</span><kbd>Space</kbd></div>
                    <div class="shortcut-row"><span class="shortcut-desc">Reveal (if typed)</span><kbd>Enter</kbd></div>
                    <div class="shortcut-row"><span class="shortcut-desc">Show hint</span><kbd>Ctrl+H</kbd></div>
                    <div class="shortcut-row"><span class="shortcut-desc">Prev blank</span><kbd>←</kbd></div>
                    <div class="shortcut-row"><span class="shortcut-desc">Next blank</span><kbd>→</kbd></div>

                    <div class="shortcut-section-title">Cards</div>
                    <div class="shortcut-row"><span class="shortcut-desc">Next card</span><kbd>Ctrl+→</kbd></div>
                    <div class="shortcut-row"><span class="shortcut-desc">Prev card</span><kbd>Ctrl+←</kbd></div>
                    <div class="shortcut-row"><span class="shortcut-desc">Skip entire card</span><kbd>Ctrl+S</kbd></div>

                    <div class="shortcut-section-title">View</div>
                    <div class="shortcut-row"><span class="shortcut-desc">Toggle fill mode</span><kbd>Ctrl+F</kbd></div>
                    <div class="shortcut-row"><span class="shortcut-desc">Toggle theme</span><kbd>Ctrl+T</kbd></div>
                    <div class="shortcut-row"><span class="shortcut-desc">Exit fill mode</span><kbd>Esc</kbd></div>
                    <div class="shortcut-row"><span class="shortcut-desc">Exit study</span><kbd>Ctrl+E</kbd></div>
                    <div class="shortcut-row"><span class="shortcut-desc">Show shortcuts</span><kbd>Ctrl+/</kbd></div>
                </div>
                <button class="btn btn-secondary" id="closeShortcutsBtn" style="margin-top:1.5rem;width:100%;justify-content:center;">Close</button>
            </div>
        </div>
    ...
    function renderCard() {
        ...
        // Reveal action bar: always visible, swap button when card is done
        const revealBar = document.getElementById('revealActionBar');
        const revealBtnEl = document.getElementById('revealBtnAction');
        const needsNormalReveal = (state.answerMode === 'reveal' || state.answerMode === 'both') && !card.allCompleted;
        const isNormalTypeOnly = state.answerMode === 'type' && !card.allCompleted;

        revealBar.style.display = 'flex'; // Always visible

        if (card.allCompleted) {
            const isLast = state.currentCard >= state.cards.length - 1;
            revealBtnEl.textContent = isLast ? '✓ Finish' : 'Next →';
            revealBtnEl.dataset.action = 'next';
        } else if (isNormalTypeOnly) {
            revealBtnEl.textContent = '⏭ Skip Word';
            revealBtnEl.dataset.action = 'reveal';
        } else {
            revealBtnEl.textContent = '👁️ Reveal Word';
            revealBtnEl.dataset.action = 'reveal';
        }

        // Adjust card body bottom padding (bar is always visible now)
        const cardBody = document.querySelector('.card-body');
        const isMobile = window.innerWidth <= 768;
        cardBody.style.paddingBottom = isMobile ? '130px' : '150px';
    }
    ...
    document.addEventListener('keydown', (e) => {
        const studyVisible = document.getElementById('cardStudyScreen').style.display === 'block' || fillMode;
        if (!studyVisible) return;
        const card = state.cards[state.currentCard];
        if (!card) return;
        const mod = e.ctrlKey || e.metaKey;

        // ── Global shortcuts (work regardless of card state) ──

        // Ctrl/Cmd+F — toggle fill mode
        if (mod && (e.key === 'f' || e.key === 'F')) {
            e.preventDefault();
            if (fillMode) exitFillMode();
            else if (state.cards.length > 0) enterFillMode();
            return;
        }

        // Escape — close shortcuts modal if open, or exit fill mode
        if (e.key === 'Escape') {
            e.preventDefault();
            const sm = document.getElementById('shortcutsModal');
            if (sm.classList.contains('active')) { sm.classList.remove('active'); return; }
            if (fillMode) { exitFillMode(); }
            return;
        }

        // Ctrl/Cmd+E — exit study (with confirm)
        if (mod && (e.key === 'e' || e.key === 'E')) {
            e.preventDefault();
            setTimeout(() => { if (confirm('Exit study session?')) backToWizard(); }, 50);
            return;
        }
        ...
        // Ctrl/Cmd+/ or Ctrl/Cmd+? — toggle keyboard shortcuts modal
        if (mod && (e.key === '/' || e.key === '?')) {
            e.preventDefault();
        ...
        // Ctrl/Cmd+Right — next card (even if not completed)
        if (mod && e.key === 'ArrowRight') {
            e.preventDefault(); nextCard(); return;
        }
        // Ctrl/Cmd+Left — previous card
        if (mod && e.key === 'ArrowLeft') {
            e.preventDefault(); prevCard(); return;
        }

        // Ctrl/Cmd+S — skip entire card (reveal all blanks)
            ...
            if (!card.allCompleted) {
                card.blanks.forEach(b => {
                    if (!b.completed) {
                        ...
                        state.missedWords.push(`${b.word} (${card.ref})`);
                    }
                });
        ...
        // ── Typing / reveal shortcuts ──
        if (state.answerMode === 'type' || state.answerMode === 'both') {
            if (e.key.length === 1 && /[a-zA-Z'']/.test(e.key) && !mod && !e.altKey) {
                e.preventDefault();
        ...
        // Arrow keys — navigate between blanks on same card
        if (e.key === 'ArrowLeft') {
            e.preventDefault();
    ...
    document.getElementById('revealBtnAction').addEventListener('click', () => {
        const action = document.getElementById('revealBtnAction').dataset.action;
        if (action === 'next') {
            const card = state.cards[state.currentCard];
            if (card && card.allCompleted && state.cards.every(c => c.allCompleted)) {
                showStats();
            } else {
                nextCard();
            }
    ...
    // ══════════════════════════════════════════════════════════════════
    let fillMode = false;
    let fillFontSize = 1.5; // rem, calculated once per session

    function calculateFillFontSize() {
        // Find the longest verse (by word count) in the session
        let maxWords = 0;
        for (const card of state.cards) {
            const wordCount = card.text.split(/\s+/).length;
            if (wordCount > maxWords) maxWords = wordCount;
        }

        // Estimate available area
        const vh = window.innerHeight;
        const vw = Math.min(window.innerWidth, 800); // cap for readability
        const availH = vh - 120; // progress bar + reveal bar + padding
        const availW = vw - 60;  // horizontal padding

        // Rough heuristic: characters per line ≈ availW / (fontSize * 0.55)
        // Lines needed ≈ totalChars / charsPerLine
        // Total height ≈ lines * fontSize * lineHeight
        // Solve for fontSize that fills availH
        const avgWordLen = 5.5; // average English word + space
        const totalChars = maxWords * avgWordLen;
        const lineHeight = 2.0;

        // Binary search for best font size
        let lo = 0.8, hi = 4.5; // rem bounds (sensible max = 4.5rem)
        for (let iter = 0; iter < 20; iter++) {
            const mid = (lo + hi) / 2;
            const pxSize = mid * 16;
            const charsPerLine = Math.floor(availW / (pxSize * 0.55));
            const lines = Math.ceil(totalChars / Math.max(charsPerLine, 1));
            const totalH = lines * pxSize * lineHeight + 60; // +60 for reference
            if (totalH < availH) lo = mid;
            else hi = mid;
        }

        fillFontSize = Math.min(lo, 4.5); // hard cap
        return fillFontSize;
    }

    function enterFillMode() {
        fillMode = true;
        calculateFillFontSize();

        document.getElementById('fillOverlay').style.display = 'flex';
        document.getElementById('cardStudyScreen').style.display = 'none';

        // Deactivate font buttons, activate fill
    ...
    function exitFillMode() {
        fillMode = false;
        document.getElementById('fillOverlay').style.display = 'none';
        document.getElementById('cardStudyScreen').style.display = 'block';

        // Restore previous font size
        document.getElementById('fillModeBtn').classList.remove('active');
        const saved = localStorage.getItem('clozeCardsFontSize') || 'md';
        document.querySelectorAll('.font-btn').forEach(b =>
            b.classList.toggle('active', b.dataset && b.dataset.size === saved)
        );
    ...
    function renderFillCard() {
        ...
        const fs = fillFontSize;
        const blankFs = fs * 0.93;

        // Reference
        const refEl = document.getElementById('fillReference');
        if (state.refMode === 'show') {
            refEl.textContent = card.ref;
            refEl.className = 'card-reference';
            refEl.style.fontSize = (fs * 0.85) + 'rem';
        } else if (state.refMode === 'hide') {
            refEl.textContent = card.ref;
            refEl.className = 'card-reference hidden';
        } else {
            refEl.textContent = card.ref;
            refEl.className = card.allCompleted ? 'card-reference revealed' : 'card-reference hidden';
            refEl.style.fontSize = (fs * 0.85) + 'rem';
        }
        ...
        words.forEach((word, i) => {
            if (blankMap.has(i)) {
                ...
                html += `<span style="white-space:nowrap;display:inline">${blank.leadingPunct}<span class="${cls}" style="font-size:${blankFs}rem;" data-bi="${blankIdx}" onclick="onBlankClick(${blankIdx})" ontouchend="onBlankTouch(event,${blankIdx})"><span class="blank-sizer" aria-hidden="true" style="font-size:${blankFs}rem;">${blank.word}</span><span class="blank-input" style="font-size:${blankFs}rem;">${display}</span></span>${blank.trailingPunct}</span> `;
            } else {
                html += `<span class="word">${word}</span> `;
            }
        });

        const verseEl = document.getElementById('fillVerseText');
        verseEl.innerHTML = html;
        verseEl.style.fontSize = fs + 'rem';
        verseEl.style.lineHeight = '2';

        // Counter & progress
        document.getElementById('fillCounter').textContent = `${state.currentCard + 1} / ${state.cards.length}`;
        const completedCards = state.cards.filter(c => c.allCompleted).length;
        document.getElementById('fillProgressBar').style.width = `${(completedCards / state.cards.length) * 100}%`;

        // Reveal bar: always visible, swap button text/action when card is complete
        const fillRevealBar = document.getElementById('fillRevealBar');
        const fillRevealBtnEl = document.getElementById('fillRevealBtn');
        const needsReveal = (state.answerMode === 'reveal' || state.answerMode === 'both') && !card.allCompleted;
        const isTypeOnly = state.answerMode === 'type' && !card.allCompleted;

        fillRevealBar.style.display = 'flex'; // Always visible

        if (card.allCompleted) {
            // Card done — show Next (or Finish on last card)
            const isLast = state.currentCard >= state.cards.length - 1;
            fillRevealBtnEl.textContent = isLast ? '✓ Finish' : 'Next →';
            fillRevealBtnEl.dataset.action = 'next';
        } else if (isTypeOnly) {
            // Type-only mode: show Skip
            fillRevealBtnEl.textContent = '⏭ Skip Word';
            fillRevealBtnEl.dataset.action = 'reveal';
        } else {
            fillRevealBtnEl.textContent = '👁️ Reveal Word';
            fillRevealBtnEl.dataset.action = 'reveal';
        }
    }
    ...
    // Fill mode exit
    document.getElementById('fillExitBtn').addEventListener('click', exitFillMode);
    document.getElementById('fillExitBtn').addEventListener('mousedown', e => e.preventDefault());

    // Fill mode reveal / next button
    document.getElementById('fillRevealBtn').addEventListener('click', () => {
        const action = document.getElementById('fillRevealBtn').dataset.action;
        if (action === 'next') {
            const card = state.cards[state.currentCard];
            if (card && card.allCompleted && state.cards.every(c => c.allCompleted)) {
                showStats();
    ...
    document.getElementById('fillRevealBtn').addEventListener('mousedown', e => e.preventDefault());

    // Fill mode tap zones
    document.getElementById('fillTapLeft').addEventListener('click', () => {
        if (state.currentCard > 0) { prevCard(); if (fillMode) renderFillCard(); }
    });
    document.getElementById('fillTapRight').addEventListener('click', () => {
        const card = state.cards[state.currentCard];
        if (card && card.allCompleted) { nextCard(); if (fillMode) renderFillCard(); }
        else if (state.currentCard < state.cards.length - 1) { nextCard(); if (fillMode) renderFillCard(); }
    });

    // Hook into existing render to update fill mode
    ...
    function backToWizard() {
        if (fillMode) { document.getElementById('fillOverlay').style.display = 'none'; fillMode = false; }
        document.getElementById('cardStudyScreen').style.display = 'none';
        document.getElementById('statsModal').classList.remove('active');
        document.getElementById('wizardScreen').style.display = 'block';
        document.getElementById('mainHeader').style.display = 'flex';
    ...
    function init() {
        ...
        // Font size buttons (exclude fill button — it has its own handler)
        document.querySelectorAll('.font-btn:not(#fillModeBtn)').forEach(btn => {
            btn.addEventListener('click', () => {
                if (fillMode) exitFillMode();
                document.querySelectorAll('.font-btn').forEach(b => b.classList.remove('active'));
                btn.classList.add('active');
        ...
        document.querySelectorAll('.font-btn:not(#fillModeBtn)').forEach(b => b.classList.toggle('active', b.dataset.size === savedFont));
        ...
        // Shortcuts modal: close button + click outside
