...
<head>
    ...
    <?php 
    $version = '3.3.0'; // MAJOR UPDATE: Study options modal, unified pile system, recent studied, removed shuffle checkbox
    ...
    function getDecksWithCategories($baseDir = 'flashcards') {
        ...
        if (empty($categories)) {
            ...
            foreach ($files as $file) {
                $cardCount = countCardsInCSV($file);
                $decks[] = [
                    'file' => basename($file),
                    'path' => $file,
    ...
    <style>        :root {
        ...
        [data-theme="candy"] {
            ...
            --accent-hover: #F0221B;
            --border: #FFD0D0;
            --shadow: rgba(210, 7, 2, 0.10);
            --shadow-hover: rgba(210, 7, 2, 0.18);
        }
...
* {
        ...
        .theme-btn[data-theme="light"] { background: linear-gradient(135deg, #FBFAF6, #FFFFFF); }
        .theme-btn[data-theme="dark"] { background: linear-gradient(135deg, #0B1026, #121F44); }
        .theme-btn[data-theme="warm"] { background: linear-gradient(135deg, #FFFBF4, #FDF3E0); }
        .theme-btn[data-theme="accent"] { background: linear-gradient(135deg, #E6EDFF, #0E44B9); }
        .theme-btn[data-theme="candy"] { background: linear-gradient(135deg, #FFE5E5, #D20702); }
        .theme-btn[data-theme="sky"] { background: linear-gradient(135deg, #DCEBFF, #0E44B9); }
        .theme-btn[data-theme="jungle"] { background: linear-gradient(135deg, #ECEFF8, #DFB757); }
        ...
        @media (max-width: 768px) {
            ...
            /* Pile mode buttons - smaller on mobile */
            .navigation.pile-mode {
                gap: 5px;
            }
...
<body data-theme="light">
    ...
    <div class="container">
        <div class="deck-selection" id="deckSelection">
            ...
            <div class="app-logo">
                <img src="pbe_team_bold_logo.png" alt="PBE Team Logo">
            </div>
                <p class="subtitle">Click any deck to start studying</p>
            </header>
            ...
            <div class="settings-section">
                ...
                <div class="setting-group">
                    ...
                    <div class="theme-selector-inline">
                        ...
                        <button class="theme-btn" data-theme="accent"></button>
                        <button class="theme-btn" data-theme="candy"></button>
                        <button class="theme-btn" data-theme="sky"></button>
                        <button class="theme-btn" data-theme="jungle"></button>
                    </div>
                </div>
                <div class="setting-group">
    ...
    <script>
        ...
        const state = {
            ...
            currentStudyMode: 'linear', // 'linear', 'shuffled', 'piles'
        ...
        function loadRecentStudied() {
            ...
            recent.slice(0, maxCount).forEach(item => {
                ...
                const deckNames = item.decks.map(d => parseFileName(d)).join(' & ');
                const studyModeLabel = item.studyMode === 'piles' ? '📚 Piles' : 
                                      item.studyMode === 'shuffled' ? '🔀 Shuffled' : '📖 Linear';
        ...
        function openStudyOptionsModal(decks) {
            ...
            document.getElementById('resumeStudyBtn').style.display = hasProgress ? 'block' : 'none';
            document.getElementById('clearProgressBtn').style.display = hasProgress ? 'block' : 'none';
        ...
        function loadDecks() {
            ...
            filteredDecks.forEach(file => {
                ...
                // Add gear icon for deck settings
                const gearBtn = document.createElement('button');
                gearBtn.className = 'deck-gear-btn';
                gearBtn.innerHTML = '⚙️';
                gearBtn.title = 'Deck Settings';
                gearBtn.onclick = e => { 
                    e.stopPropagation(); 
                    openDeckSettings(file);
                };
                ...
                card.onclick = e => {
                    if (e.target !== checkbox && 
                        !e.target.classList.contains('deck-gear-btn') && 
        ...
        async function loadDeck(files, studyMode = 'linear') {
            ...
            // Save to recent studied
            saveRecentStudied(files, studyMode);
            
            // Set shuffle based on study mode
            state.settings.shuffle = (studyMode === 'shuffled');
            
            // Set pile mode based on study mode
            state.settings.pileMode = (studyMode === 'piles');
            
            // Load pile data using unified key
            loadPileData();

            for (const file of files) {
                // Use deckPaths to get the correct path (supports folder structure)
        ...
        function initEvents() {
            ...
            document.getElementById('pile1Btn').onclick = () => sortCardToPile(1);
            document.getElementById('pile2Btn').onclick = () => sortCardToPile(2);
            document.getElementById('pile3Btn').onclick = () => sortCardToPile(3);
            ...
            document.getElementById('restartBtn').onclick = () => {
                if (confirm('Restart deck from beginning?')) {
                    if (state.settings.pileMode) {
                        // In pile mode, return to pile selection
                        showPileSelectionScreen();
                    } else {
                        // Regular mode - restart cards
                        state.currentIndex = 0;
                        state.flipped = false;
                        state.cards = [...state.allCards];
                        state.totalCards = state.cards.length;
